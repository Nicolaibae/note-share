import { IAuth, AuthModel } from "../models/auth.model";
...
export class authService {
    async Register(data: IAuth): Promise<IAuth> {

        if (!data.username) throw new Error("vui long dien ten")
        const userAuth = await AuthModel.findOne({ username: data.username })
        if (userAuth) throw new Error("đã tồn tại tên này")
        if (!data.password) throw new Error("vui long dien password")
        const hashPassword = await bcrypt.hash(data.password, 10)
        const newUser = new AuthModel({ username: data.username, password: hashPassword })
        await newUser.save()

        return newUser

    }
    async Login(data: IAuth): Promise<IAuth> {
        if (!data.username) throw new Error("vui long dien ten")
        const user = await AuthModel.findOne({ username: data.username })
    ...
    async RefreshToken(data: IAuth): Promise<IAuth> {
    ...
    async Logout(data: IAuth): Promise<IAuth>{
        if (!data || !data.refreshtoken) throw new Error("Không tồn tại token");
            const decoded: any = jwt.verify(data.refreshtoken,process.env.REFRESH_TOKEN as string)
        if(!decoded) throw new Error("decode ko thành công")
            const user = await AuthModel.findById(decoded.id)
    ...
    async Profile (data: IAuth): Promise<IAuth>{
        if (!data || !data.id) throw new Error("Không tồn tại token");
        const user = await AuthModel.findById(data.id).select("-password -accesstoken -refreshtoken")
